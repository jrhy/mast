...
type iterItem[K,V comparable] struct {
	considerLink interface{}
	yield        entry
}

func (m *Mast[K,V]) diff(
	ctx context.Context,
	oldMast *Mast[K,V],
	...
	var oldStack iterItemStack[K,V]
	if oldMast != nil {
		oldStack = newIterItemStack[K,V](iterItem[K,V]{considerLink: oldMast.root})
	}
	newStack := newIterItemStack[K,V](iterItem[K,V]{considerLink: m.root})
	for {
		if m.debug {
...
func (m *Mast[K,V]) alreadyNotified(ctx context.Context, name string, linkByHeight map[uint8]interface{}, link interface{}) bool {
	path := []interface{}{}
	myLink := link
	var keyHeight uint8
	for {
...
type iterItemStack[K,V comparable] struct {
	things []iterItem[K,V]
}

func newIterItemStack[K,V comparable](item iterItem[K,V]) iterItemStack[K,V] {
	return iterItemStack[K,V]{
		[]iterItem[K,V]{item},
	}
}

func (stack *iterItemStack[K,V]) pop() *iterItem[K,V] {
...
func (stack *iterItemStack[K,V]) pushNode(node *mastNode[K,V]) {
...
func (stack *iterItemStack[K,V]) pushLink(link interface{}) {
	if link != nil {
		stack.push(&iterItem[K,V]{considerLink: link})
	}
}

func (stack *iterItemStack[K,V]) pushYield(node *mastNode[K,V], i int) {
	stack.push(&iterItem[K,V]{yield: entry{node.Key[i], node.Value[i]}})
}

func (stack *iterItemStack[K,V]) push(item *iterItem[K,V]) {
