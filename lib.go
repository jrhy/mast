...
type mastNode[K,V comparable] struct {
	Key      []K
	Value    []V
...
type pathEntry[K,V comparable] struct {
	node      *mastNode[K,V]
	linkIndex int
}
...
func split[K,V comparable](ctx context.Context, node *mastNode[K,V], key interface{}, mast *Mast[K,V]) (leftLink, rightLink interface{}, err error) {
	var splitIndex int
...
type findOptions[K,V comparable] struct {
	targetLayer        uint8
	currentHeight      uint8
...
func (node *mastNode[K,V]) follow(ctx context.Context, i int, createOk bool, mast *Mast[K,V]) (*mastNode[K,V], error) {
	if node.Link[i] != nil {
		...
		child := emptyNodePointer[K,V](cap(node.Key))
		node.Link[i] = child
		return child, nil
	}
}
...
func emptyNode[K,V comparable](branchFactor int) mastNode[K,V] {
...
func emptyNodePointer[K,V comparable](branchFactor int) *mastNode[K,V] {
	node := emptyNode[K,V](branchFactor)
	return &node
}

func (node *mastNode[K,V]) extract(from, to int) *mastNode[K,V] {
	newChild := emptyNode[K,V](cap(node.Key))
	newChild.Key = append([]K{}, node.Key[from:to]...)
	newChild.Value = append([]V{}, node.Value[from:to]...)
...
func (m *Mast[K,V]) grow(ctx context.Context) error {
	...
	newNode := emptyNode[K,V](int(m.branchFactor))
	start := 0
...
func validateNode[K,V comparable](ctx context.Context, node *mastNode[K,V], mast *Mast[K,V]) {
