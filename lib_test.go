...
func newTestTree[K,V comparable](zeroKey K, zeroValue V) Mast[K,V] {
	return Mast[K,V]{
		root:            emptyNodePointer[K,V](DefaultBranchFactor),
		zeroKey:         zeroKey,
		zeroValue:       zeroValue,
...
func TestSplit(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	m := NewInMemory[int,string]()
	node := mastNode[int,string]{
		Key:   []int{10, 20, 30},
		Value: []string{"", "", ""},
...
func TestInsert(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	m := NewInMemory[int,int]()
	err := m.Insert(ctx, 50, 50)
	require.NoError(t, err)
	node, err := m.load(ctx, m.root)
	require.NoError(t, err)
	require.Equal(t, []int{50}, node.Key)
	require.Equal(t, []int{50}, node.Value)
	...
	node, err = m.load(ctx, m.root)
	require.NoError(t, err)
	require.Equal(t, []int{40, 50}, node.Key)
	require.Equal(t, []int{40, 50}, node.Value)
	...
	node, err = m.load(ctx, m.root)
	require.NoError(t, err)
	require.Equal(t, []int{40, 50, 60}, node.Key)
	require.Equal(t, []int{40, 50, 60}, node.Value)
	...
	node, err = m.load(ctx, m.root)
	require.NoError(t, err)
	require.Equal(t, []int{40, 45, 50, 60}, node.Key)
	require.Equal(t, []int{40, 45, 50, 60}, node.Value)
...
func apply(root *Mast[uint,uint], to []TestOperation) error {
	for _, to := range to {
...
func TestEmptyLeavesCongruence(t *testing.T) {
	t.Parallel()
	const testLen = 300
	congruenceCase := make([]uint, testLen)
	for i := 0; i < testLen; i++ {
		n := uint(i * 16)
		congruenceCase[i] = n
	}
...
func checkDiff(t *testing.T, oldOps []TestOperation, newOps []TestOperation) bool {
	...
	err := apply(&old, oldOps)
	require.NoError(t, err)

	new := newTestTree(uint(0), uint(0))
	err = apply(&new, newOps)
	require.NoError(t, err)
...
func checkCongruence[K comparable, V comparable](t *testing.T, baseTree Mast[K,V], keys []K) bool {
	m, err := baseTree.Clone(ctx)
	require.NoError(t, err)
	m2, err := baseTree.Clone(ctx)
	require.NoError(t, err)
	for _, key := range keys {
		err := m.Insert(ctx, key, m.zeroValue)
	...
	for _, key := range keys {
		err := m2.Insert(ctx, key, m.zeroValue)
		assert.NoError(t, err)
		if err != nil {
			return false
		}
...
func TestRemoteExample(t *testing.T) {
	...
	root := NewRoot(nil)
	m, err := LoadMast[int,string](ctx, root, &remoteConfig)
	require.NoError(t, err)
...
func TestStructValues(t *testing.T) {
	...
	root := NewRoot(nil)
	m, err := LoadMast[int,foo](ctx, root, &remoteConfig)
	require.NoError(t, err)
...
func TestStringKeys(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	m, err := LoadMast[string,int](ctx, NewRoot(nil), &RemoteConfig{
		KeysLike:                "hi",
		ValuesLike:              5,
		StoreImmutablePartsWith: NewInMemoryStore(),
	})
...
func TestNilValues(t *testing.T) {
	...
	{
		v := 5
		require.NoError(t, m.Insert(ctx, "hey", &v))
	}
...
func (m *Mast[K,V]) toSlice(ctx context.Context) ([]entry, error) {
...
func (m *Mast[K,V]) keys(ctx context.Context) ([]interface{}, error) {
...
func (m *Mast[K,V]) contains(ctx context.Context, key K) (bool, error) {
	if m.root == nil {
		return false, nil
	}
	node, err := m.load(ctx, m.root)
...
func TestEmbeddedArrayValue(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	m, err := LoadMast[string,*TEAV](ctx, NewRoot(nil), &RemoteConfig{
		KeysLike:                "hi",
		StoreImmutablePartsWith: NewInMemoryStore(),
	})
	require.NoError(t, err)
...
func TestCustomMarshal(t *testing.T) {
	...
	m, err := LoadMast[string,value](ctx, NewRoot(nil), &RemoteConfig{
		StoreImmutablePartsWith: NewInMemoryStore(),
		Marshal:                 marshalGob,
