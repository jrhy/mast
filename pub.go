...
func (m *Mast[K,V]) Delete(ctx context.Context, key K, value V) error {
	if m.debug {
	...
	node, i, err := findEntry[K,V](ctx, m, key, value, &options)
	if err != nil {
		return err
	}
...
func findEntry[K,V comparable](ctx context.Context, m *Mast[K,V], key K, value V, options *findOptions[K,V]) (*mastNode[K,V], int, error) {
...
func deleteEntry[K,V comparable](ctx context.Context, m *Mast[K,V], node *mastNode[K,V], i int) (*mastNode[K,V], error) {
	oldNode := node
...
func (m *Mast[K,V]) flush(ctx context.Context) (string, error) {
	if m.persist == nil {
...
func (m *Mast[K,V]) Get(ctx context.Context, k K, value *V) (bool, error) {
	if m.root == nil {
		return false, nil
	}
	node, err := m.load(ctx, m.root)
...
func (m *Mast[K,V]) Insert(ctx context.Context, key K, value V) error {
	if m.debug {
...
func (m *Mast[K,V]) Iter(ctx context.Context, f func(interface{}, interface{}) error) error {
	node, err := m.load(ctx, m.root)
	if err != nil {
		return err
	}
...
func (m *Mast[K,V]) SeekIter(ctx context.Context, k interface{}, f func(interface{}, interface{}) error) error {
	if m.root == nil {
		return nil
	}
	node, err := m.load(ctx, m.root)
...
//func (r *Root) LoadMast[K,V any](ctx context.Context, config *RemoteConfig) (*Mast[K,V], error) {
func LoadMast[K,V comparable](ctx context.Context, r *Root, config *RemoteConfig) (*Mast[K,V], error) {
	var link interface{}
	if r.Link != nil {
		link = *r.Link
	} else {
		link = emptyNodePointer[K,V](int(r.BranchFactor))
	}
	shrinkSize := uint64(1)
...
func (m *Mast[K,V]) MakeRoot(ctx context.Context) (*Root, error) {
...
func NewInMemory[K,V comparable]() Mast[K,V] {
	return Mast[K,V]{
...
func (m *Mast[K,V]) Height() uint8 {
	return m.height
}

// Size returns the number of entries in the tree.
func (m *Mast[K,V]) Size() uint64 {
	return m.size
}

// Clone() returns a new Mast that shares all the source's data
// but can evolve independently (copy-on-write).
func (m *Mast[K,V]) Clone(ctx context.Context) (Mast[K,V], error) {
...
func (m *Mast[K,V]) IsDirty() bool {
	if node, ok := m.root.(*mastNode[K,V]); ok {
		return node.dirty
	}
	return false
}
